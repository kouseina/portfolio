  ...
  return (
    <Link data-variant={product.type} className={cn([base, border, background], `hover:from-[${hoverColor}] hover:via-gradient-gray duration-500`)} href={product.link} target="_blank">
      <div className={`absolute inset-0 z-10 s-full duration-500 overflow-hidden rounded-2xl sm:rounded-xl`}>
        <Image quality={100} priority className={`object-cover object-top duration-500  ${type == 'work' ? 'opacity-50 group-hover:opacity-80' : 'opacity-70 group-hover:opacity-90'} group-hover:scale-[101%]`} src={product.image ? product.image : DefaultImage} width={1000} height={700} alt={product.name} />
      </div>
